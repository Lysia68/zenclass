...
function CheckoutForm({ planName, price, isSetup, trialDaysLeft, onSuccess, onCancel }: {
  planName:string; price:number; isSetup:boolean; trialDaysLeft:number;
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div style={{padding:"14px 16px",background:C.accentBg,borderRadius:10,border:"1px solid rgba(160,104,56,.2)",marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div>
          <div style={{fontSize:14,fontWeight:700,color:C.text}}>Formule {planName}</div>
          <div style={{fontSize:12,color:C.textSoft,marginTop:2}}>
            {isSetup && trialDaysLeft > 0
        ...
        <div style={{fontSize:20,fontWeight:800,color:C.accent}}>
          {isSetup && trialDaysLeft > 0 ? "0€" : `${price}€`}
      ...
      <div style={{display:"flex",gap:10,marginTop:20}}>
        ...
        <button type="submit" disabled={!stripe||loading}
          style={{flex:2,padding:"11px",background:"linear-gradient(145deg,#B88050,#9A6030)",border:"none",borderRadius:9,color:"#fff",fontSize:14,fontWeight:700,cursor:loading?"wait":"pointer",opacity:loading?0.7:1}}>
          {loading ? "Traitement…" : isSetup && trialDaysLeft > 0 ? "Enregistrer ma carte" : `Payer ${price}€/mois`}
        </button>
      </div>
...
// ── Modal ─────────────────────────────────────────────────────────────────────
function PaymentModal({ plan, clientSecret, intentType, trialDaysLeft, onSuccess, onClose }: {
  plan:typeof PLANS[0]; clientSecret:string; intentType:"payment"|"setup";
  trialDaysLeft:number; onSuccess:()=>void; onClose:()=>void;
}) {
  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()}
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:32,width:"100%",maxWidth:480,boxShadow:"0 32px 64px rgba(42,31,20,.15)",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <Elements stripe={stripePromise} options={{clientSecret, appearance:stripeAppearance, locale:"fr"}}>
          <CheckoutForm planName={plan.name} price={plan.price} isSetup={intentType==="setup"} trialDaysLeft={trialDaysLeft} onSuccess={onSuccess} onCancel={onClose}/>
        </Elements>
      </div>
    </div>
  )
...
// ── Page ──────────────────────────────────────────────────────────────────────
type Studio = { id:string; name:string; billing_status:string; trial_ends_at:string|null; plan_slug:string; stripe_customer_id:string|null; stripe_subscription_id:string|null }
...
export default function BillingPage() {
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Inter',-apple-system,sans-serif"}}>
      ...
      {modal&&<PaymentModal plan={modal.plan} clientSecret={modal.clientSecret} intentType={modal.intentType} trialDaysLeft={trialDaysLeft} onSuccess={handlePaymentSuccess} onClose={()=>setModal(null)}/>}
      ...
      <div style={{maxWidth:860,margin:"0 auto",padding:"40px 24px"}}>
          ...
          {PLANS.map(plan=>{
            ...
            return (
              <div key={plan.slug} style={{background:C.surface,border:`${isCurrent?"2.5px":"1.5px"} solid ${isCurrent?plan.color:C.border}`,borderRadius:16,padding:"24px 20px",position:"relative",boxShadow:isCurrent?`0 4px 20px ${plan.color}22`:"none"}}>
                {plan.popular&&!isCurrent&&<div style={{position:"absolute",top:-11,left:"50%",transform:"translateX(-50%)",background:plan.color,color:"#fff",fontSize:11,fontWeight:700,padding:"3px 12px",borderRadius:12,whiteSpace:"nowrap"}}>⭐ Populaire</div>}
        ...
        <div style={{padding:"16px 20px",background:C.surface,border:`1px solid ${C.borderSoft}`,borderRadius:12,fontSize:12,color:C.textSoft,lineHeight:1.7}}>
          ...
          Paiement sécurisé par Stripe. Annulable à tout moment depuis le portail de facturation.
          Les 15 premiers jours sont offerts — votre carte ne sera débitée qu'à l'issue de la période d'essai.
        </div>
      </div>
    </div>
  )
